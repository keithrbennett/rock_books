...
module RockBooks
  ...
  RSpec.describe RockBooks::Journal do
    ...
    let(:test_date) { Date.iso8601('2018-05-13') }
    ...
    let(:fn_journal_from_string) { ->(string) { Journal.from_string(chart_of_accounts, string) } }

    it "can be instantiated" do
      journal = fn_journal_from_string.('')
    ...
    it 'can read its title' do
      ...
      data = '@title: ' + title
      journal = fn_journal_from_string.(data)
    ...
    it 'can read its date prefix' do
      ...
      expect(Journal.from_string(chart_of_accounts, data).date_prefix).to eq(date_prefix)
    end

    it 'correctly handles doc_type' do
      doc_type = 'journal'
      data = '@doc_type: ' + doc_type
      expect(Journal.from_string(chart_of_accounts, data).doc_type).to eq(doc_type)
    end
    ...
    it 'can produce JSON which can then be parsed and entries contain the expect values' do
      ...
      journal = Journal.from_string(chart_of_accounts, data)
    ...
    it %q{raises an error when an journal's main account code does not exist} do
      ...
      expect { Journal.from_string(chart_of_accounts, data) }.to raise_error(AccountNotFoundError)
    end
    ...
    it 'raises the correct negative error when an unbalanced transaction is specified in debit journal' do
      ...
      create_bad_journal = -> { Journal.from_string(chart_of_accounts, data) }
