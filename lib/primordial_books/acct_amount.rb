...
class AcctAmount < Struct.new(:date, :acct_id, :amount)
  ...
  def self.parse_tokens(date, tokens, total_amount)
    if tokens.size == 1
      [AcctAmount.new(date, tokens.first, total_amount)]
    elsif tokens.size.odd?
      raise "Token list size must be 1 or even: #{tokens}"
    else
      tokens.each_slice(2).map { |pair| AcctAmount.new(date, pair.first, pair.last)}
    end
  end
end
end
